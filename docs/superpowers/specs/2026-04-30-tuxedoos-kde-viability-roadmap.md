...
| W4b | 2 | CI dependency-list fix | `done` | `w4b-ci-dependency-fix` (merged) | `2026-05-02-w4b-ci-dependency-fix-design.md` | `2026-05-02-w4b-ci-dependency-fix.md` | yes | Adds `clang`, `libclang-dev`, `libssl-dev`, `libpipewire-0.3-dev` to `.github/workflows/ci.yml`; removes dead `libgtk4-layer-shell-dev`; adds `--locked` + `cargo build --release`; folds in 4 CI-hygiene additions (timeout, permissions, concurrency, CARGO_TERM_COLOR). **Code landed correctly; CI red on `cargo fmt --check` due to pre-existing W4c-territory rustfmt drift in upstream `app/src/*.rs` (transcript_log.rs:733, app.rs:18/51/106/192, cli.rs:374, etc.). Drift would have failed on the next CI run regardless of W4b. W4c naturally fixes via `rust-toolchain.toml` pinning.** Run: github.com/lukepatrick/pepper-x/actions/runs/25268093310. **Caveat resolved 2026-05-03 by W4c** (rust-toolchain.toml pinning + cargo fmt + workspace [lints] config). CI green on the next push to origin/main after W4c lands. |
| W4c | 2 | Pin Rust toolchain version | `in-progress` | `w4c-rust-toolchain-pin` | `2026-05-03-w4c-rust-toolchain-pin-design.md` | `2026-05-03-w4c-rust-toolchain-pin.md` | yes | **New 2026-05-01 from W1 findings.** Add `rust-toolchain.toml` with a pinned stable version. Upstream's `dtolnay/rust-toolchain@stable` is a moving target; with rustc 1.95.0, upstream main fails both `cargo fmt --check` and `cargo clippy -- -D warnings`. Pinning fixes the drift. |
...
- `2026-05-03` — **W4b done with caveats** (matches W1 pattern). Code landed correctly: ci.yml has the 4 added apt packages (`clang`, `libclang-dev`, `libssl-dev`, `libpipewire-0.3-dev`), `libgtk4-layer-shell-dev` removed, `--locked` on cargo invocations, new `cargo build --release` step, and 4 hygiene additions (timeout-minutes, permissions, concurrency, CARGO_TERM_COLOR). **CI red** on the first run after push (run 25268093310): `cargo fmt --check` failed on upstream-authored files (`app/src/transcript_log.rs:733`, `app/src/app.rs:18/51/106/192`, `app/src/cli.rs:374`, etc.). Diffs are line-wrapping-only; consistent with rustfmt rolling forward (likely 1.94 → 1.95) on `dtolnay/rust-toolchain@stable`. **This drift is pre-existing**, not caused by W4b — it would have failed on the next CI run regardless. W4c naturally fixes by pinning a specific stable Rust release via `rust-toolchain.toml` (the upstream code is fmt-clean against that pinned version). Side observation: `actions/checkout@v4` is on Node.js 20 which is being deprecated June 2026; non-blocking, fold into W4c or a tiny follow-up. State: `in-progress` → `done (caveats)`. **current_workstream advances to W4c**.
